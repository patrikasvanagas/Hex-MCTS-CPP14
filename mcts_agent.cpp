...
MCTSAgent::MCTSAgent(double exploration_constant,
	std::chrono::milliseconds move_time_limit, bool verbose) : exploration_constant(exploration_constant),
	move_time_limit(move_time_limit), verbose(verbose), gen(rd()) {}

MCTSAgent::Node::Node(char player, std::pair<int, int> move,
	std::shared_ptr<Node> parent) : wins(0),
	visits(0),
	move(move),
	player(player),
	children(),
...
void MCTSAgent::expand_root(const std::shared_ptr<Node>& node, const Board& board)
{
